...
pub enum TimeDeviation {
    ...
    IncorrectProjectAllocation {
        date: NaiveDate,
        message: String,
    },
    IncorrectAbsenceAllocation {
        date: NaiveDate,
        project: String,
    },
    AdjacentWffToAbsence {
        wff_date: NaiveDate,
...
impl TimeValidationService {
    ...
    pub fn validate_work_hours(
        ...
        if scheduled_hours == 0.0 && employee_config.is_international {
            ...
            if let Some(service) = &registration.service {
                if service.id != "16" {
                    // "16" is the "Other" service code
                    return Err(TimeDeviation::IncorrectProjectAllocation {
                        date,
                        message:
                            "International employees should use Service 16 for Swedish holidays"
                                .to_string(),
                    });
                }
            }
    ...
    pub fn has_absence_for_remaining_hours(
        &self,
        registration: &DetailedRegistration,
        scheduled_hours: f64,
    ) -> bool {
    ...
    pub fn validate_absence_allocation(
        &self,
        registration: &DetailedRegistration,
        employee_config: &EmployeeConfig,
    ) -> Result<(), TimeDeviation> {
        if registration.registration_code.type_ == "ABSENCE" {
            ...
            if let Some(project) = &registration.project {
                if project.id != employee_config.main_project {
                    let date =
                        NaiveDate::parse_from_str(&registration.worked_date, "%Y-%m-%d").unwrap();
    ...
    pub fn validate_wff_adjacent_to_absence(
        &self,
        ...
        for reg in registrations {
            ...
            if reg.registration_code.type_ == "ABSENCE" {
                ...
                if service.id == "52" {
                    // "52" is the "Work Free Friday" service code
    ...
    pub fn mark_week_complete(&mut self, employee_id: &str, year: i32, week: i32) {
        ...
        self.week_statuses
            .insert((employee_id.to_string(), year, week), status);
    }
    ...
    pub fn mark_month_complete(&mut self, employee_id: &str, year: i32, month: i32) {
        ...
        self.month_statuses
            .insert((employee_id.to_string(), year, month), status);
    }
    ...
    pub fn check_weekly_reminders(&self) {
        ...
        if today.weekday() == Weekday::Mon {
            let prev_week = if current_week > 1 {
                current_week - 1
            } else {
                52
            }; // Handle year wraparound
            let prev_year = if current_week > 1 {
                current_year
            } else {
                current_year - 1
            };
            ...
            for (employee_id, _) in &self.employee_configs {
                    ...
                    || matches!(self.week_statuses.get(&key), Some(WeekStatus::Incomplete))
                {
                    // Send reminder
                    info!(
                        "REMINDER: {} needs to complete time reporting for week {} of {}",
        ...
        for ((employee_id, year, month), status) in &self.month_statuses {
            if let MonthStatus::HasDeviations(deviations) = status {
                for deviation in deviations {
                    ...
                    let today = Local::now().date_naive();
                    let month_completion_deadline =
                        NaiveDate::from_ymd_opt(*year, *month as u32, 7).unwrap();
    ...
    fn send_deviation_notification(&self, employee_id: &str, deviation: &TimeDeviation) {
        match deviation {
            TimeDeviation::MissingMandatoryField { field, date } => {
                info!(
                    "NOTIFICATION to {}: Missing mandatory field '{}' for date {}",
                    employee_id, field, date
                );
            }
            TimeDeviation::IncorrectWorkHours {
                date,
                scheduled,
                reported,
            } => {
            ...
            TimeDeviation::MissingFlexTime {
                date,
                missing_hours,
            } => {
                info!(
                    "NOTIFICATION to {}: Missing flex time+ for {} extra hours on {}",
                    employee_id, missing_hours, date
                );
            }
            TimeDeviation::IncorrectProjectAllocation { date, message } => {
                info!(
                    "NOTIFICATION to {}: Incorrect project allocation on {}. {}",
            ...
            TimeDeviation::AdjacentWffToAbsence {
                wff_date,
                absence_date,
                absence_type,
            } => {
