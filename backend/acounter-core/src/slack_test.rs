...
async fn test_server() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let oauth_listener_config = SlackOAuthListenerConfig::new(
        slack_morphism::SlackClientId(env::var("SLACK_CLIENT_ID").unwrap()),
        slack_morphism::SlackClientSecret(env::var("SLACK_CLIENT_SECRET").unwrap()),
        env::var("SLACK_BOT_SCOPE").unwrap(),
        env::var("SLACK_REDIRECT_HOST").unwrap(),
    );
    ...
    let slack = axum::routing::Router::new()
        .nest(
            "/auth",
    ...
    let app = axum::routing::Router::new().nest("/api/slack", slack);
